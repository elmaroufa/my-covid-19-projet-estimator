...
const powerToDays = (days) => Math.trunc(days / 3);


// function calcul currentlyInfected
const calculCurrentlyInfected = (reportedCases, multiple) => reportedCases * multiple;
...
const calculInfectionByRequestedTime = (currentlyInfected, mypowerToDays) => {
	return currentlyInfected * (2 ** mypowerToDays);
}
...
*/
const calculDollarsInFlight = (infectionByRequestedTime, 
	periodeDay, avgDailyIncomeInUSD, avgDailyIncomePopulation) => {
	return Math.trunc(infectionByRequestedTime * periodeDay * avgDailyIncomePopulation * avgDailyIncomeInUSD);
}

const covid19ImpactEstimator = (data) => {
	
  const daysCacul = convertTimeToElaoseToDay(data.periodType, data.timeToElapse);
  const mypowerToDays = powerToDays(daysCacul);
  ...
  */
  const valCurrentlyInfectedIp = calculCurrentlyInfected(data.reportedCases, 10);
  const valCurrentlyInfectedSi = calculCurrentlyInfected(data.reportedCases, 50);
  const valInfectionByRequestedTimeIp = calculInfectionByRequestedTime(valCurrentlyInfectedIp, mypowerToDays);
  const valInfectionByRequestedTimeSi = calculInfectionByRequestedTime(valCurrentlyInfectedSi, mypowerToDays);
  const valSevereCaseByrequestedTimeIp = calculSevereCaseByrequestedTime(valInfectionByRequestedTimeIp);
  const valSevereCaseByrequestedTimeSi = calculSevereCaseByrequestedTime(valInfectionByRequestedTimeSi);
	const valHospitalBedsByRequestedTimeIp = caclulHospitalBedsByRequestedTime(data.totalHospitalBeds, 
		valSevereCaseByrequestedTimeIp);
	const valHospitalBedsByRequestedTimeSi = caclulHospitalBedsByRequestedTime(data.totalHospitalBeds, 
		valSevereCaseByrequestedTimeSi);
  const valCasesForICUByRequestedTimeIp = calculcasesForICUByRequestedTime(valInfectionByRequestedTimeIp);
  const valCasesForICUByRequestedTimeSi = calculcasesForICUByRequestedTime(valInfectionByRequestedTimeSi);
  const valcasesForVentilatorsByRequestedTimeIp = calculcasesForVentilatorsByRequestedTime(valInfectionByRequestedTimeIp);
  const valcasesForVentilatorsByRequestedTimeSi = calculcasesForVentilatorsByRequestedTime(valInfectionByRequestedTimeSi);
	const valdollarsInFlightIp = calculDollarsInFlight(valInfectionByRequestedTimeIp, daysCacul, 
		data.region.avgDailyIncomeInUSD, data.region.avgDailyIncomePopulation);
	const valdollarsInFlightSi = calculDollarsInFlight(valInfectionByRequestedTimeSi, daysCacul, 
		data.region.avgDailyIncomeInUSD, data.region.avgDailyIncomePopulation);
  return {
			data: data,
    	estimate: {
